...
**Visibility:** The shell renders the entire active floor unconditionally — there is no fog-of-war, no reveal mask, and no shader overlay. Terrain tints come from [`TerrainVisualColor`](../src/Core/Maps/TerrainVisualColor.cs) sampled at world coordinates per cell. [`GameRoot`](../src/Godot/GameRoot.cs) rasterizes the **visible cell window** into a cached `ImageTexture` when the cull rect expands, the active floor `Z`, zoom, or cell size changes, then draws that texture once per `_Draw` and paints grid lines on top (nearest texture filter for crisp pixels).
