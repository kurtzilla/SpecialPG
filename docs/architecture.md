...
**Shell tuning:** [`config.ini`](../src/Godot/config.ini) (loaded by [`ShellAppConfig`](../src/Godot/ShellAppConfig.cs)) supplies values such as `cell_size_px`, default map dimensions (used for bounded bootstrap / procedural defaults), **`wasd_steps_per_second`** (discrete WASD repeat rate; clamped **1..1024** sub-tile steps per second), **`wasd_max_sub_steps_per_physics_frame`** (burst cap per physics tick, clamped **1..256**), **`max_land_bridge_cells`** (optional cap on the procedural origin-to-largest-landmass bridge; **0** = unlimited), zoom limits, **`randomize_startup_seed`**, and optional **`profile_shell_draw`**, without recompiling. The **right preset stack** on [`ShellHudLayer`](../src/Godot/ShellHudLayer.cs) exposes sliders that adjust the same WASD values at runtime and persist them to `config.ini` without restarting. Bounded procedural fills use [`ProceduralWorldMapGenerator`](../src/Core/Maps/ProceduralWorldMapGenerator.cs), which ends with [`ForceLandWalkMargin`](../src/Core/Maps/ForceLandWalkMargin.cs) so sub-tile walkability stays consistent one tile beyond `ForceLand` patches and the land-bridge path. The [`ShellPlayer`](../src/Godot/ShellPlayer.cs) node **lerps** its `Position` toward the authoritative sub-cell center each `_Process` so the camera scroll feels smoother than the Core step rate; Core sync and foot readouts use **`AuthoritativeFootWorld`**. **Physics cadence:** [`GameRoot`](../src/Godot/GameRoot.cs) drives WASD in **`_PhysicsProcess`**; Godot’s default **`physics_ticks_per_second`** is **60** unless you change it under **Project → Project Settings → Physics → Common** (or add `[physics]` keys in `project.godot`). Mismatches between physics rate and display refresh can exaggerate stepping; tune `wasd_steps_per_second` first, then physics FPS if needed. Window mode stays in `project.godot`. The checked-in [`sample_twofloor.json`](../src/Godot/maps/sample_twofloor.json) uses its own `width`/`height` from the file (kept smaller than those defaults so the asset stays tractable in git); bump dimensions in `scripts/gen_sample_twofloor.py` when you need a full-size sample on disk.

**Visibility:** The shell renders the entire active floor unconditionally — there is no fog-of-war, no reveal mask, and no shader overlay. [`GameRoot`](../src/Godot/GameRoot.cs) maintains one [`TerrainChunkView`](../src/Godot/Terrain/TerrainChunkView.cs) per visible 32×32 map chunk under [`TerrainFloorLayer`](../src/Godot/Terrain/TerrainFloorLayer.cs) (`ZIndex = -8`); [`SurfaceFloorLayer`](../src/Godot/Terrain/SurfaceFloorLayer.cs) (`ZIndex = -4`) holds decor and entities above ground. Chunks rebuild when dirty or newly entering the camera cull. Grid lines are still drawn in `GameRoot._Draw`. **Default:** chunk pixels use [`TerrainVisualColor`](../src/Core/Maps/TerrainVisualColor.cs) via [`TileMainPatchPlanner`](../src/Core/Maps/Rendering/TileMainPatchPlanner.cs). **Optional:** with `terrain_use_sprites=true` and a loaded atlas, [`TerrainChunkRasterizer`](../src/Godot/Terrain/TerrainChunkRasterizer.cs) blits main patches via [`TerrainBakeRasterizer.PaintOp`](../src/Godot/Terrain/TerrainBakeRasterizer.cs).

### Terrain rendering (Shell)

**Phase 0 (contracts):** Core classifies each cell into a [`TerrainRenderCategory`](../src/Core/Maps/Rendering/TerrainRenderCategory.cs) via [`TerrainAppearance`](../src/Core/Maps/Rendering/TerrainAppearance.cs) (shared with [`TerrainVisualColor`](../src/Core/Maps/TerrainVisualColor.cs)). [`TileSpriteResolver`](../src/Core/Maps/Rendering/TileSpriteResolver.cs) emits [`TileDrawOp`](../src/Core/Maps/Rendering/TileDrawOp.cs) records (grid origin, atlas key, draw layer). The Shell owns [`TerrainAtlasCatalog`](../src/Godot/Terrain/TerrainAtlasCatalog.cs) and `res://art/terrain/terrain_atlas.png` (placeholder tiles; regenerate via [`scripts/gen_terrain_placeholder_atlas.py`](../scripts/gen_terrain_placeholder_atlas.py)).
...
**Main patches (Phase 3):** [`TileMainPatchPlanner`](../src/Core/Maps/Rendering/TileMainPatchPlanner.cs) places non-overlapping `Main4x4`, `Main2x2`, and `Main1x1` ops over each planning rectangle. Anchors use global grid alignment (`gx % 4 == 0` for 4×4, `gx % 2 == 0` for 2×2) so adjacent chunks pick the same patch at boundaries. An ownership grid prevents double coverage; each patch requires a uniform [`TerrainRenderCategory`](../src/Core/Maps/Rendering/TerrainRenderCategory.cs) under its cells (sampled at cell centers). Patch size at an anchor is chosen deterministically from `worldSeed` and anchor `(gx, gy)` via weighted roll (`MainPatchWeights`: 4→1, 2→2, 1→4). Chunk rebuild expands the planning rect by [`TransitionMarginCells`](../src/Godot/Terrain/TerrainChunkRasterizer.cs) (= 1) and clips painted pixels to the chunk interior. Atlas bands per category: 1×1 strip (32px), 2×2 strip (64px), 4×4 strip (128px) — see [`TerrainAtlasCatalog`](../src/Godot/Terrain/TerrainAtlasCatalog.cs).

| `TerrainDrawLayer` | Typical categories |
|--------------------|------------------|
| `Water` | Deep/shallow water, forced water |
| `GroundNatural` | Land, coast, hill, blocked, forced land, empty |
| `GroundArtificial` | (future built floors) |
| `Decor` | Procedural decor scatter (`DecorFloorLayer`) |

**Surface layers (Phase 5):** [`DecorScatterPlanner`](../src/Core/Maps/Rendering/DecorPlacement.cs) places sparse decor on land/coast/hill cells (~3% deterministic scatter). [`DecorFloorLayer`](../src/Godot/Terrain/DecorFloorLayer.cs) syncs visible chunks when `decor_enabled=true` (default on in checked-in `config.ini`). [`EntityFloorLayer`](../src/Godot/Terrain/EntityFloorLayer.cs) draws [`EntityStore`](../src/Core/Maps/EntityStore.cs) props on the active floor (`Z == ActorZ`), sorted by **Y** then **X**; `EntityKinds.Actor` is skipped (player uses [`ShellPlayer`](../src/Godot/ShellPlayer.cs)). `MarkMapCellDirty` refreshes terrain and decor in neighboring chunks; entity spawn/destroy fires `EntityStore.EntityChanged`.

**3D quads (optional later):** Same `TileDrawOp` list could drive flat `QuadMesh` tiles under an orthographic `Camera3D` (melange variant C). Core stays unchanged; only the Shell drawer swaps.

**Sort order** when multiple ops overlap: `Layer`, then `OriginGy`, then `OriginGx` via [`TileDrawOpComparer`](../src/Core/Maps/Rendering/TileDrawOpComparer.cs) before chunk paint.
